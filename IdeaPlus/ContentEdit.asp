...
%>
		...
		<table cellpadding="0" cellspacing="0">
		  <tr>
			<th>单页面信息：添加，修改企业相关的内容</th>
		  </tr>
		  <tr>
			<td align="center">【<%If Result = "Add" then%>添加<%ElseIf Result = "Modify" then%>修改<%End If%>单页面信息】</td>
		  </tr>
		</table>
		<br>
		<table border="0" cellpadding="0" cellspacing="0">
		  <form name="editForm" method="post" action="ContentEdit.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>">
		  <tr>
			<td>
		...
		&nbsp;不少于1个字符</td>
              ...
              <tr>
				...
				<td><input name="AddTime" type="text" class="textfield" id="AddTime" style="WIDTH: 240;" value="<%if AddTime<>"" then Response.Write(""&AddTime&"") else Response.Write(""&now()&"")%>" maxlength="100">
				&nbsp;请按 <%=now%> 格式正确输入</td>
			  </tr>
			  <tr>
...
sub ContentEdit()
  ...
  if Action="SaveEdit" then '保存编辑信息
    ...
    if Result="Add" then '添加
	  sql="select * from IdeaNet_Content"
	...
	rs.close
    set rs=nothing 
    response.write "<script language=javascript> alert('成功编辑单页面信息！');location.replace('ContentList.asp');</script>"
  else '提取单页面信息
