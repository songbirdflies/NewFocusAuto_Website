...
function PlaceFlag()
  if Result="Search" then
    Response.Write "商品/作品：列表&nbsp;->&nbsp;检索&nbsp;->&nbsp;添加时间[<font color='red'>"&StartDate&"至"&EndDate&"</font>]，关键字[<font color='red'>"&Keyword&"</font>]"
  else
    if SortPath<>"" then
      Response.Write "商品/作品：列表&nbsp;->&nbsp<a href='ProductsList.asp'>全部</a>"
	  TextPath(SortID)
	else
      Response.Write "商品/作品：列表&nbsp;->&nbsp全部"
	end if
  end if
end function  
%>
...
<table cellpadding="0" cellspacing="0">
  <tr>
    <th>商品/作品检索及分类查看：添加，修改，删除商品/作品</th>
  </tr>
  <tr>
    <td height="36" align="center"><table class="noborder" cellspacing="0">
      <tr>
        ...
        <td align="right" nowrap><a href="ProductsList.asp" onClick='changeAdminFlag("商品/作品列表")'>全部商品/作品</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="ProductsSort.asp" onClick='changeAdminFlag("选择查看分类")'>其他类别信息</a></td>
      </tr>
    </table></td>    
  </tr>
</table>
...
<table cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <th colspan="2" width="120">操作
      ...
      <input onClick="CheckProducts(this.form)" name="buttonProductselect" type="button" class="button"  id="submitProductselect" value="反" style="HEIGHT: 18px;WIDTH: 16px;">	</th>
  </tr>
  <% ProductsList() %>
  </form>
</table>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
	  ...
	  Response.Write "<td nowrap><a href='ProductsList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""商品/作品列表"")'>"&SortText(rs("SortID"))&"</a></td>" & vbCrLf
	  if StrLen((rs("TitleNameLangI")))>30 then
        Response.Write "<td nowrap><a href='ProductsEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""商品/作品列表"")'>"&StrLeft(rs("TitleNameLangI"),30)&"</a></td>" & vbCrLf
      else
        Response.Write "<td nowrap><a href='ProductsEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""商品/作品列表"")'>"&rs("TitleNameLangI")&"</a></td>" & vbCrLf
      end if 
      ...
      Response.Write "<td width=100 nowrap><a href='ProductsEdit.asp?Result=Add' onClick='changeAdminFlag(""添加商品/作品"")'><font color='#330099'>添加</font></a>&nbsp;&nbsp;<a href='ProductsEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改商品/作品"")'><font color='#330099'>修改</font></a>&nbsp;&nbsp;<a href='ProductsList.asp?Result=ModifyOrderNum&ID="&rs("ID")&"' onClick='changeAdminFlag(""排序商品/作品"")'><font color='#330099'>排序</font></a></td>" & vbCrLf
    ...
    Response.Write "<td colspan='2' nowrap  bgcolor='#EBF2F9'><input name='submitDelSelect' type='button' class='button'  id='submitDelSelect' value='删除所选' onClick='ConfirmDel(""您真的要删除这些商品/作品吗？"");'></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
    response.write "<tr><td height='50' align='center' colspan='11' nowrap  bgcolor='#EBF2F9'>暂无商品/作品</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
...
Function TextPath(ID)
  ...
  rs.open sql,conn,1,1
  TextPath="&nbsp;->&nbsp;<a href=ProductsList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&">"&rs("SortNameLangI")&"</a>"
...
sub ModifyOrderNum()
  ...
  response.write "<form action='ProductsList.asp?Result=SaveOrderNum' method='post' name='formOrderNum'>"
  response.write "<tr>"
  response.write "<td height='24' align='center' nowrap  bgcolor='#EBF2F9'>ID：<input name='ID' type='text' class='textfield'  style='WIDTH: 30;' value='"&ID&"' maxlength='4' readonly>&nbsp;商品/作品名称：<input name='TitleNameLangI' type='text' class='textfield' id='TitleNameLangI' style='WIDTH: 240;' value='"&TitleNameLangI&"' maxlength='36' >&nbsp;排序号：<input name='OrderNum' type='text' class='textfield' style='WIDTH: 60;' value='"&OrderNum&"' maxlength='4'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&OrderNum&"';}"">*(数字越小越靠前)&nbsp;&nbsp;点击量：<input name='Hits' type='text' class='textfield' style='WIDTH: 60;' value='"&Hits&"' maxlength='8'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&Hits&"';}"">&nbsp;&nbsp;<input name='submitOrderNum' type='submit' class='button' value='保存' style='WIDTH: 60;' ></td>"
  response.write "</tr>"
  response.write "</form>"
...
sub SaveOrderNum()
  dim rs,sql
  set rs = server.createobject("adodb.recordset")
  sql="select * from IdeaNet_Products where ID="& request.form("ID")
