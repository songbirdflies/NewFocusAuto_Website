...
<%
if Instr(session("AdminPurview"),"|61,")=0 then 
...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From IdeaNet_InfoSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from  IdeaNet_InfoSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from  IdeaNet_Info where Instr(SortPath,'"&SortPath&"')>0")
    response.write ("<script language=javascript> alert('成功删除本类、子类及所有下属信息条目，点击确定查看类别树！');location.replace('InfoSort.asp');</script>")
  Case "Save"
	saveFolder ()
  Case "Edit"
	editFolder
...
<table cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td height="24" align="center" nowrap  bgcolor="#EBF2F9"><a href="InfoSort.asp?Action=Add&ParentID=0">添加一级分类</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="InfoList.asp" onClick='changeAdminFlag("信息中心列表")'>查看所有信息中心</a></td>
  </tr>
  <tr>
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From IdeaNet_InfoSort where ParentID="&id&" order by id"
  rs.open sql,conn,1,1
  ...
  while not rs.eof
    ChildCount=conn.execute("select count(*) from IdeaNet_InfoSort where ParentID="&rs("id"))(0)
    ...
    response.write("<font color='#FF0000'>分类：</font><a href='InfoSort.asp?Action=Add&ParentID="&rs("id")&"'>添加</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='InfoSort.asp?Action=Edit&ID="&rs("id")&"'>修改</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='InfoSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortNameLangI="&rs("SortNameLangI")&"&SortPath="&rs("SortPath")&"'>移</a>")
	...
	response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href=javascript:ConfirmDelSort('InfoSort',"&rs("id")&")>删除</a>")
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;<font color='#FF0000'>信息中心：</font><a href='InfoEdit.asp?Result=Add' onClick='changeAdminFlag(""添加信息中心"")'>添加</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='InfoList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""信息中心列表"")'>列表</a>&nbsp;|&nbsp;排序&nbsp;"&rs("OrderNum")&"")
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From IdeaNet_InfoSort where ID="&ParentID
    rs.open sql,conn,1,1
	ParentPath=rs("SortPath")
  end if
%>
<table cellpadding="0" cellspacing="0">
<form name="editForm" method="post" action="InfoSort.asp?Action=Save&From=Add">
  <tr>
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then 
    sql="Select * From IdeaNet_InfoSort"
  ...
  rs.update 
  response.write ("<script language=javascript> alert('"&Action&"保存成功，点击确定查看类别树！');location.replace('InfoSort.asp');</script>")
End Function 
'修改节点---------------------------------
Function editFolder()
  Dim ID
...
%>
<table cellpadding="0" cellspacing="0">
<form name="editForm" method="post" action="InfoSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
  <tr>
...
<form name="MoveForm" method="post" action="InfoSort.asp?Action=MoveSave">
  <tr>
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From IdeaNet_InfoSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update IdeaNet_InfoSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新类别数字路径
  conn.execute("update IdeaNet_InfoSort set ParentID='"&toID&"' where ID="&fromID)'更新类别父类ID
  conn.execute("update IdeaNet_Info set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新信息数字路径
  response.write ("<script language=javascript> alert('移动类别成功，点击确定查看类别树！');location.replace('InfoSort.asp');</script>")
End Function
%>
