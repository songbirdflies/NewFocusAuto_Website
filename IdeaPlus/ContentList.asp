...
<table cellpadding="0" cellspacing="0">
  <tr>
    <th><strong>单页内容信息：添加，修改单页内容相关的内容</strong></th>
  </tr>
  <tr>
    <td height="24" align="center">【单页内容信息管理】</td>    
  </tr>
</table>
<br>
<table border="0" cellpadding="0" cellspacing="0">
  <form action="DelContent.asp?Result=Content" method="post" name="formDel" >
    <tr>
      <th width="30">ID</th>
      <th>信息名称</th>
      <th width="60">显示顺序</th>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
	  ...
	  Response.Write "<td nowrap>&nbsp;<a href='ContentEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改单页内容信息"")'>"&rs("TitleNameLangI")&"</a></td>" & vbCrLf    
      ...
      Response.Write "<td width=100 nowrap><a href='ContentEdit.asp?Result=Add' onClick='changeAdminFlag(""添加单页内容信息"")'><font color='#330099'>添加</font></a>&nbsp;&nbsp;<a href='ContentEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改单页内容信息"")'><font color='#330099'>修改</font></a>&nbsp;&nbsp;<a href='ContentList.asp?Result=ModifyOrderNum&ID="&rs("ID")&"' onClick='changeAdminFlag(""排序单页内容信息"")'><font color='#330099'>排序</font></a></td>" & vbCrLf
    ...
    Response.Write "<td colspan='2' nowrap  bgcolor='#EBF2F9'><input name='submitDelSelect' type='button' class='button'  id='submitDelSelect' value='删除所选' onClick='ConfirmDel(""您真的要删除这些单页内容信息吗？"");'></td>" & vbCrLf
    Response.Write "</tr>" & vbCrLf
  else
    response.write "<tr><td height='50' align='center' colspan='7' nowrap  bgcolor='#EBF2F9'>暂无单页内容信息</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
...
sub ModifyOrderNum()
  ...
  set rs = server.createobject("adodb.recordset")
  sql="select * from IdeaNet_Content where ID="& ID
  rs.open sql,conn,1,1
  ...
  response.write "<form action='ContentList.asp?Result=SaveOrderNum' method='post' name='formOrderNum'>"
  response.write "<tr>"
  response.write "<td height='24' align='center' nowrap  bgcolor='#EBF2F9'>ID：<input name='ID' type='text' class='textfield'  style='WIDTH: 30;' value='"&ID&"' maxlength='4' readonly>&nbsp;单页内容信息名称：<input name='TitleNameLangI' type='text' class='textfield' id='TitleNameLangI' style='WIDTH: 240;' value='"&TitleNameLangI&"' maxlength='36' >&nbsp;排序号：<input name='OrderNum' type='text' class='textfield' style='WIDTH: 60;' value='"&OrderNum&"' maxlength='4'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&OrderNum&"';}"">*(数字越小越靠前)&nbsp;&nbsp;点击量：<input name='Hits' type='text' class='textfield' style='WIDTH: 60;' value='"&Hits&"' maxlength='8'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&Hits&"';}"">&nbsp;&nbsp;<input name='submitOrderNum' type='submit' class='button' value='保存' style='WIDTH: 60;' ></td>"
  response.write "</tr>"
  response.write "</form>"
...
sub SaveOrderNum()
  dim rs,sql
  set rs = server.createobject("adodb.recordset")
  sql="select * from IdeaNet_Content where ID="& request.form("ID")
  ...
  rs.close
  set rs=nothing
  response.redirect "ContentList.asp"
end sub
%>