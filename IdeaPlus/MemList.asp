...
<table cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td height="24" align="center">【会员组别信息管理&nbsp;&nbsp;&nbsp;操作：<a href="MemEdit.asp?Result=Add" onClick='changeAdminFlag("添加新会员")'>添加会员</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="MemList.asp" onClick='changeAdminFlag("查看所有会员")'>查看所有会员</a>】</td>    
  </tr>
</table>
<br>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
      ...
      Response.Write "<td width=100 nowrap><a href='MemEdit.asp?Result=Add' onClick='changeAdminFlag(""添加会员组别信息"")'><font color='#330099'>添加</font></a>&nbsp;&nbsp;<a href='MemEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改会员组别信息"")'><font color='#330099'>修改</font></a>&nbsp;&nbsp;<a href='MemList.asp?Result=ModifyOrderNum&ID="&rs("ID")&"' onClick='changeAdminFlag(""排序会员组别信息"")'><font color='#330099'>排序</font></a></td>" & vbCrLf
    ...
    response.write "<tr><td height='50' align='center' colspan='11' nowrap  bgcolor='#EBF2F9'>暂无会员组别信息</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
  Response.Write "<td colspan='11'>" & vbCrLf
...
<%
sub ModifyOrderNum()
  dim rs,sql,ID,MemName,OrderNum
  ...
  response.write "<form action='MemList.asp?Result=SaveOrderNum' method='post' name='formOrderNum'>"
  response.write "<tr>"
  response.write "<td height='24' align='center' nowrap  bgcolor='#EBF2F9'>ID：<input name='ID' type='text' class='textfield'  style='WIDTH: 30;' value='"&ID&"' maxlength='4' readonly>&nbsp;会员组别信息名称：<input name='MemName' type='text' class='textfield' id='MemName' style='WIDTH: 240;' value='"&MemName&"' maxlength='36' >&nbsp;排序号：<input name='OrderNum' type='text' class='textfield' style='WIDTH: 60;' value='"&OrderNum&"' maxlength='4'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&OrderNum&"';}"">*(数字越小越靠前)&nbsp;&nbsp;<input name='submitOrderNum' type='submit' class='button' value='保存' style='WIDTH: 60;' ></td>"
  response.write "</tr>"
  response.write "</form>"
...
sub SaveOrderNum()
  dim rs,sql
  set rs = server.createobject("adodb.recordset")
  sql="select * from IdeaNet_Members where ID="& request.form("ID")
