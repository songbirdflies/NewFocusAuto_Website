...
Select Case Action
  ...
  Case "Del"
    ...
    sql="Select * From IdeaNet_AdsSort where ID="&request.QueryString("id")
    rs.open sql,conn,1,1
	SortPath=rs("SortPath")
	conn.execute("delete from  IdeaNet_AdsSort where Instr(SortPath,'"&SortPath&"')>0")
    conn.execute("delete from  IdeaNet_Ads where Instr(SortPath,'"&SortPath&"')>0")
    response.write ("<script language=javascript> alert('成功删除本类、子类及所有下属信息条目，点击确定查看类别树！');location.replace('AdsSort.asp');</script>")
  Case "Save"
	saveFolder ()
  Case "Edit"
	editFolder
...
<table cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td height="24" align="center" nowrap  bgcolor="#EBF2F9"><a href="AdsSort.asp?Action=Add&ParentID=0">添加一级分类</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="AdsList.asp" onClick='changeAdminFlag("广告列表")'>查看所有广告</a></td>
  </tr>
  <tr>
...
Function Folder(id)
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From IdeaNet_AdsSort where ParentID="&id&" order by id"
  rs.open sql,conn,1,1
  ...
  while not rs.eof
    ChildCount=conn.execute("select count(*) from IdeaNet_AdsSort where ParentID="&rs("id"))(0)
    ...
    response.write("<font color='#FF0000'>分类：</font><a href='AdsSort.asp?Action=Add&ParentID="&rs("id")&"'>添加</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='AdsSort.asp?Action=Edit&ID="&rs("id")&"'>修改</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='AdsSort.asp?Action=Move&ID="&rs("id")&"&ParentID="&rs("Parentid")&"&SortNameLangI="&rs("SortNameLangI")&"&SortPath="&rs("SortPath")&"'>移</a>")
	...
	response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href=javascript:ConfirmDelSort('AdsSort',"&rs("id")&")>删除</a>")
    response.write("&nbsp;&nbsp;&nbsp;&nbsp;<font color='#FF0000'>广告：</font><a href='AdsEdit.asp?Result=Add' onClick='changeAdminFlag(""添加广告"")'>添加</a>")
    response.write("<font color='#367BDA'>&nbsp;|&nbsp;</font><a href='AdsList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""广告列表"")'>列表</a>&nbsp;|&nbsp;排序&nbsp;"&rs("OrderNum")&"")
...
Function addFolderForm(ParentID)
  ...
  if ParentID=0 then
    ...
    Set rs=server.CreateObject("adodb.recordset")
    sql="Select * From IdeaNet_AdsSort where ID="&ParentID
    rs.open sql,conn,1,1
	ParentPath=rs("SortPath")
  end if
%>
<table cellpadding="0" cellspacing="0">
<form name="editForm" method="post" action="AdsSort.asp?Action=Save&From=Add">
  <tr>
  ...
  <tr>
	...
	<table class="noborder" width="100%" border="0" cellpadding="0" cellspacing="0">
	  ...
	  <tr>
	    ...
	    <td colspan="5">
		<input name="Width" type="text" maxlength="25" id="Width" class="textfield" style="width:40px;" dataType="Require"  msg="请填写宽度" /> Px &nbsp;&nbsp; (0为不限宽)
	    </td>
      </tr>
	  <tr>
	    <td align="right">广告高度：</td>
	    <td colspan="5">
		<input name="Height" type="text" maxlength="25" id="Height" class="textfield" style="width:40px;" dataType="Require"  msg="请填写高度" /> Px &nbsp;&nbsp; (0为不限高)
	    </td>
      </tr>
	  <tr>
...
Function saveFolder
  ...
  Set rs=server.CreateObject("adodb.recordset")
  if From="Add" then 
    sql="Select * From IdeaNet_AdsSort"
  ...
  rs.update 
  response.write ("<script language=javascript> alert('"&Action&"保存成功，点击确定查看类别树！');location.replace('AdsSort.asp');</script>")
End Function 
'修改节点---------------------------------
Function editFolder()
  Dim ID
...
Function editFolderForm(ID)
  Dim SortNameLangI,ViewFlagLangI,SortNameLangE,ViewFlagLangE,SortNameLangD,ViewFlagLangD,Width,Height,ParentID,SortPath,rs,sql,OrderNum,SmallPic,BigPic,ConciseLangI,ConciseLangD,ConciseLangE,GotoUrl
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From IdeaNet_AdsSort where ID="&ID
  rs.open sql,conn,1,1
...
%>
<table cellpadding="0" cellspacing="0">
<form name="editForm" method="post" action="AdsSort.asp?Action=Save&From=Edit&ID=<%=ID%>">
  <tr>
  ...
  <tr>
	...
	<table class="noborder" cellpadding="0" cellspacing="0">
	  ...
	  <tr>
	    <td align="right">广告高度：</td>
	    <td colspan="5">
		<input name="Height" type="text" maxlength="25" id="Height" class="textfield" style="width:40px;" value="<%=Height%>" dataType="Require"  msg="请填写高度" /> Px &nbsp;&nbsp; (0为不限高)
	    </td>
      </tr>
	  <tr>
        <td align="right">分类小图：</td>
...
<form name="MoveForm" method="post" action="AdsSort.asp?Action=MoveSave">
  <tr>
...
Function saveMoveFolder()
  ...
  Set rs=server.CreateObject("adodb.recordset")
  sql="Select * From IdeaNet_AdsSort where ID="&fromParentID
  rs.open sql,conn,0,1
  fromParentSortPath=rs("SortPath")
  conn.execute("update IdeaNet_AdsSort set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新类别数字路径
  conn.execute("update IdeaNet_AdsSort set ParentID='"&toID&"' where ID="&fromID)'更新类别父类ID
  conn.execute("update IdeaNet_Ads set SortPath='"&toSortPath&"'+Mid(SortPath,Len('"&fromParentSortPath&"')+1) where Instr(SortPath,'"&fromSortPath&"')>0")'更新信息数字路径
  response.write ("<script language=javascript> alert('移动类别成功，点击确定查看类别树！');location.replace('AdsSort.asp');</script>")
End Function
%>
