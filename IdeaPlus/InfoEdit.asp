...
dim i,ID,TitleNameLangI,TitleNameLangD,TitleNameLangE,ViewFlagLangI,ViewFlagLangD,ViewFlagLangE,HtmlUrlNameLangI,HtmlUrlNameLangD,HtmlUrlNameLangE,SearchKeywordsLangI,SearchKeywordsLangD,SearchKeywordsLangE,SortNameLangI,SeoKeywordsLangI,SeoKeywordsLangD,SeoKeywordsLangE,SeoDescriptionLangI,SeoDescriptionLangD,SeoDescriptionLangE,SortID,SortPath,SourceLangI,SourceLangD,SourceLangE,OrderNum,TmpID
...
<table cellpadding="0" cellspacing="0">
  <tr>
    <th>信息中心检索及分类查看：添加，修改，删除信息中心信息</th>
  </tr>
  <tr>
    <td height="24" align="center">【<%If Result = "Add" then%>添加<%ElseIf Result = "Modify" then%>修改<%End If%>信息中心信息】</td>
  </tr>
</table>
<br>
<table cellpadding="0" cellspacing="0">
  <form name="editForm" method="post" onSubmit="return selectall();" action="InfoEdit.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>">
  <tr>
    <td height="24">
    ...
    <tr class="LangShowE">
      ...
      <tr>
        ...
        <td><input name="SortNameLangI" type="text" class="textfield" id="SortNameLangI" value="<%=SortNameLangI%>" style="width:240px;background-color:#EBF2F9;" readonly>&nbsp;<a href="javaScript:OpenScript('SelectSort.asp?Result=Info',500,500,'')"><img src="Images/Select.gif" width="30" height="16" border="0" align="absmiddle"></a></td>
      </tr>
      <tr>
      ...
      <tr>
		   ...
		   sa=split(SortClass,",")
			For i = LBound(sa) To UBound(sa)
			 ...
			 dim rspt,sqlpt
				...
				sqlpt="Select * from IdeaNet_InfoSort where Id="&Trim(sa(i))&""
		...
		sub printSort(id,str)
			dim sql,rs
			sql="select * from IdeaNet_InfoSort where ParentID="&id
...
%>
	...
	<input type="button" name="button" onClick="AddPic();" value="添加图片"><br>
		...
		<%  dim sizevalue
			...
			sql2="select * from IdeaNet_InfoPic where ShowID="&Request("ID")&" Order by Id "
...
sub InfoEdit()
  ...
  if Action="SaveEdit" then '保存编辑信息中心信息
    set rs = server.createobject("adodb.recordset")
    ...
    if Result="Add" then '创建信息中心信息
	  sql="select * from IdeaNet_Info"
	...
	if Result="Modify" then '修改信息中心信息
	...
	set rs=server.CreateObject("adodb.recordset")
	sql="select * from IdeaNet_InfoPic where ShowID="&TmpID&" Order by id "
	rs.open sql,conn,1,3
	...
	rs.close
    set rs=nothing
    response.write "<script language=javascript> alert('成功编辑信息中心信息！');location.replace('InfoList.asp');</script>"
  else '提取信息中心信息
