  ...
  <tr>
    <td height="36" align="center"><table class="noborder" cellspacing="0">
      <tr>
        ...
        <td align="right" nowrap><a href="NewsList.asp" onClick='changeAdminFlag("新闻列表")'>全部新闻</a><font color="#0000FF">&nbsp;|&nbsp;</font><a href="NewsSort.asp" onClick='changeAdminFlag("选择查看分类")'>其他类别信息</a></td>
      </tr>
    </table></td>    
  </tr>
</table>
...
<table cellpadding="0" cellspacing="0">
  ...
  <tr>
    ...
    <th colspan="2" width="120">操作
      ...
      <input onClick="CheckNews(this.form)" name="buttonNewselect" type="button" class="button"  id="submitNewselect" value="反" style="HEIGHT: 18px;WIDTH: 16px;">	</th>
  </tr>
  <% NewsList() %>
  </form>
</table>
...
'-----------------------------------------------------------
  if(idcount>0 and sqlid<>"") then'如果记录总数=0,则不处理
    ...
    while(not rs.eof)'填充数据到表格
	  ...
	  Response.Write "<td nowrap><a href='NewsList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&"' onClick='changeAdminFlag(""新闻列表"")'>"&SortText(rs("SortID"))&"</a></td>" & vbCrLf
	  if StrLen((rs("TitleNameLangI")))>30 then
        Response.Write "<td nowrap><a href='NewsEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""新闻列表"")'>"&StrLeft(rs("TitleNameLangI"),30)&"</a></td>" & vbCrLf
      else
        Response.Write "<td nowrap><a href='NewsEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""新闻列表"")'>"&rs("TitleNameLangI")&"</a></td>" & vbCrLf
      end if 
      ...
      Response.Write "<td width=100 nowrap><a href='NewsEdit.asp?Result=Add' onClick='changeAdminFlag(""添加新闻"")'><font color='#330099'>添加</font></a>&nbsp;&nbsp;<a href='NewsEdit.asp?Result=Modify&ID="&rs("ID")&"' onClick='changeAdminFlag(""修改新闻"")'><font color='#330099'>修改</font></a>&nbsp;&nbsp;<a href='NewsList.asp?Result=ModifyOrderNum&ID="&rs("ID")&"' onClick='changeAdminFlag(""排序新闻"")'><font color='#330099'>排序</font></a></td>" & vbCrLf
    ...
    response.write "<tr><td height='50' align='center' colspan='11' nowrap  bgcolor='#EBF2F9'>暂无新闻</td></tr>"
  end if
'-----------------------------------------------------------
'-----------------------------------------------------------
  Response.Write "<tr>" & vbCrLf
...
Function TextPath(ID)
  ...
  rs.open sql,conn,1,1
  TextPath="&nbsp;->&nbsp;<a href=NewsList.asp?SortID="&rs("ID")&"&SortPath="&rs("SortPath")&">"&rs("SortNameLangI")&"</a>"
...
sub ModifyOrderNum()
  ...
  response.write "<form action='NewsList.asp?Result=SaveOrderNum' method='post' name='formOrderNum'>"
  response.write "<tr>"
  response.write "<td height='24' align='center' nowrap  bgcolor='#EBF2F9'>ID：<input name='ID' type='text' class='textfield'  style='WIDTH: 30;' value='"&ID&"' maxlength='4' readonly>&nbsp;新闻名称：<input name='TitleNameLangI' type='text' class='textfield' id='TitleNameLangI' style='WIDTH: 240;' value='"&TitleNameLangI&"' maxlength='36' >&nbsp;排序号：<input name='OrderNum' type='text' class='textfield' style='WIDTH: 60;' value='"&OrderNum&"' maxlength='4'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&OrderNum&"';}"">*(数字越小越靠前)&nbsp;&nbsp;点击量：<input name='Hits' type='text' class='textfield' style='WIDTH: 60;' value='"&Hits&"' maxlength='8'  onKeyDown='if(event.keyCode==13)event.returnValue=false' onchange=""if(/\D/.test(this.value)){alert('只能在排序号内输入整数！');this.value='"&Hits&"';}"">&nbsp;&nbsp;<input name='submitOrderNum' type='submit' class='button' value='保存' style='WIDTH: 60;' ></td>"
  response.write "</tr>"
  response.write "</form>"
...
sub SaveOrderNum()
  dim rs,sql
  set rs = server.createobject("adodb.recordset")
  sql="select * from IdeaNet_News where ID="& request.form("ID")
