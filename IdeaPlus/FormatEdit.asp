...
%>
		...
		<table cellpadding="0" cellspacing="0">
		  <tr>
			<th>显示格式信息：添加，修改显示格式相关的内容</th>
		  </tr>
		  <tr>
			<td align="center">【<%If Result = "Add" then%>添加<%ElseIf Result = "Modify" then%>修改<%End If%>显示格式信息】</td>
		  </tr>
		</table>
		<br>
		<table border="0" cellpadding="0" cellspacing="0">
		  <form name="editForm" method="post" action="FormatEdit.asp?Action=SaveEdit&Result=<%=Result%>&ID=<%=ID%>">
		  <tr>
			<td>
...
sub FormatEdit()
  ...
  if Action="SaveEdit" then '保存编辑信息
    ...
    if Result="Add" then '添加
	  sql="select * from IdeaNet_Format"
	...
	rs.close
    set rs=nothing 
    response.write "<script language=javascript> alert('成功编辑显示格式信息！');location.replace('FormatList.asp');</script>"
  else '提取显示格式信息
